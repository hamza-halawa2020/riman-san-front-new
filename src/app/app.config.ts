            ...
            return http
                ...
                .pipe(
                    map((data: any) => {
                        ...
                        const serverVersion = data.version[lang] || '0'; // إصدار السيرفر للغة
...
export function initializeApp(translate: TranslateService, http: HttpClient): () => Promise<void> {
    return () => {
        const defaultLang = 'en';
        const translationKey = `i18n_${defaultLang}`;
        const versionKey = `i18n_version_${defaultLang}`;
        ...
        return http.get(`${environment.backEndUrl}/translations/index`).toPromise()
            .then((data: any) => {
                const translations = data.translations[defaultLang] || {};
                const serverVersion = data.version[defaultLang] || '0';

                if (cachedTranslations && cachedVersion === serverVersion) {
                    translate.setTranslation(defaultLang, JSON.parse(cachedTranslations));
                } else {
                    translate.setTranslation(defaultLang, translations);
                    localStorage.setItem(translationKey, JSON.stringify(translations));
                    localStorage.setItem(versionKey, serverVersion);
                }
                translate.use(defaultLang);
